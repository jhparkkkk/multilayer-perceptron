...
def split_and_save_data(data, seed=42, test_size=0.2, save_dir='data'):
    ...
    # combine x and y 
    train_data = pd.concat([pd.DataFrame(X_train_resampled, columns=X.columns), pd.Series(y_train_resampled, name='diagnosis')], axis=1)
    valid_data = pd.concat([pd.DataFrame(X_valid_scaled, columns=X.columns), pd.Series(y_valid, name='diagnosis')], axis=1)
